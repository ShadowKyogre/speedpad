...
class TestInputBox(CursesTestCase):
    ...
    def test_putch(self):
        ...
        # backspace strips back to last tabstop
        self.box.reset()
        # tabsize = 8 (default)        13  16 19
        #                               |  |  |
        for ch in str2ord("0123456789012     x"): self.box.putch(ch)
        #                  |      |    |  |<-- should stop before (16)
        #                  0      7   12  15
        ypos, xpos = self.box.pad.getyx()
        self.assertEqual((ypos, xpos), (0, 19))
        self.box.putch(curses.ascii.BS)
        ypos, xpos = self.box.pad.getyx()
        self.assertEqual((ypos, xpos), (0, 18))
        self.box.putch(curses.ascii.BS)
        ypos, xpos = self.box.pad.getyx()
        self.assertEqual((ypos, xpos), (0, 16))
        self.box.putch(curses.ascii.SP)
        ypos, xpos = self.box.pad.getyx()
        self.assertEqual((ypos, xpos), (0, 17))
        self.box.putch(curses.ascii.BS)
        ypos, xpos = self.box.pad.getyx()
        self.assertEqual((ypos, xpos), (0, 16))
        self.box.putch(curses.ascii.BS)
        ypos, xpos = self.box.pad.getyx()
        self.assertEqual((ypos, xpos), (0, 13))
        self.box.putch(curses.ascii.BS)
        ypos, xpos = self.box.pad.getyx()
        self.assertEqual((ypos, xpos), (0, 12))
        # backspace strips back to last tabstop
        self.box.reset()
        self.box.tabsize = 4
        # tabsize = 4          4   8  12  16
        #                      |   |   |   |
        for ch in str2ord("0123456           x"): self.box.putch(ch)
        # tabstops:           |   |   |   |
        ypos, xpos = self.box.pad.getyx()
        self.assertEqual((ypos, xpos), (0, 19))
        self.box.putch(curses.ascii.BS)
        ...
        self.box.putch(curses.ascii.BS)
        ypos, xpos = self.box.pad.getyx()
        self.assertEqual((ypos, xpos), (0, 8))
        self.box.putch(curses.ascii.BS)
        ypos, xpos = self.box.pad.getyx()
        self.assertEqual((ypos, xpos), (0, 7))
        self.box.putch(curses.ascii.BS)
        self.box.putch(curses.ascii.BS)
        self.box.putch(curses.ascii.BS)
        ypos, xpos = self.box.pad.getyx()
        self.assertEqual((ypos, xpos), (0, 4))
        # backspace strips back to last character
        self.box.reset()
        self.box.tabsize = 0
        for ch in str2ord("0123456           x"): self.box.putch(ch)
        ypos, xpos = self.box.pad.getyx()
